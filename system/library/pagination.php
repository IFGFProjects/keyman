...
class Pagination {
	...
	public function render() {
		...
		// $output = '<ul class="pagination"><li><a href="#totop" id="totop"><i class="glyphicon glyphicon-arrow-up"></i></a></li>';
		...
		if ($num_pages > 1) {
			...
			for ($i = $start; $i <= $end; $i++) {
				if ($page == $i) {
					...
					if($i == 1){
						$clear_page_param_replace_from = array('&page={page}','?page={page}');
						$output .= '<li><a href="' . str_replace($clear_page_param_replace_from, '', $this->url) . '">' . $i . '</a></li>';
					} else{
