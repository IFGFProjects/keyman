	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$option_data = array();
				// echo "<h1 style='color:red;'>cart['option'] :</h1><pre>";print_r($cart['option']);echo "</pre><hr>";
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
				if (is_object($value))
				{
					$loader->model('catalog/product');
					$mdl = $registry->get('model_catalog_product');
					$kit_prod=$mdl->getProduct($product_option_id);
// echo "<h1 style='color:red;'>value :</h1><pre>";print_r($value);echo "</pre><hr>";
					foreach ($value as $product_option_id2 => $value2) 
					{
						$option_query = $this->db->query("SELECT po.product_option_id, po.option_id, od.name, o.type FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_option_id = '" . (int)$product_option_id2 . "'  AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");


						$kit_prod_id=$product_option_id;

						$option_value_query = $this->db->query("SELECT pov.option_value_id, ovd.name, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_option_value_id = '" . (int)$value2 . "' AND pov.product_option_id = '" . (int)$product_option_id2 . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
						...
						if ($option_value_query->num_rows) {
							...
							$option_data[] = array(
								'product_id'       		  => $kit_prod['product_id'],
								'product_option_id'       => $product_option_id2,
								'product_option_value_id' => $value2,
								'option_id'               => $option_query->row['option_id'],
								'option_value_id'         => $option_value_query->row['option_value_id'],
								'name'                    => $kit_prod['name']." (".$option_query->row['name'].")",
				...
				{
					$price=round($price-($price*($this->session->data['customer_buydiscount']/100)));
				}

				// echo "<h1 style='color:red;'>$price :</h1><pre>";print_r($product_query->row);echo "</pre><hr>";
