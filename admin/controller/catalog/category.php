...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		// echo "<h1 style='color:red;'>data['categories'] :</h1><pre>";print_r($data['categories']);echo "</pre><hr>";
	...
	protected function getForm() {
		...
		if (isset($this->request->post['image_mobile'])) {
			...
			$data['image_mobile'] = $category_info['image_mobile'];
		} else {
			$data['image_mobile'] = '';
		}
		...
		} elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['image_mobile'])) {
			$data['thumb_mobile'] = $this->model_tool_image->resize($category_info['image_mobile'], 100, 100);
		} else {
		...
		if (isset($this->request->post['is_complect'])) {
			$data['is_complect'] = $this->request->post['is_complect'];
		} elseif (!empty($category_info)) {
			$data['is_complect'] = $category_info['is_complect'];
		} else {
			$data['is_complect'] = 0;
		}
		...
		// HCV
		$this->load->language("module/h_custom_variables");
		$data['hcv_tab_name']=$this->language->get('hcv_pc_main_tab_text');
		$this->load->model("hcv/product_category");
		if (isset($this->request->get['category_id']))
			{$data['hcv_content']=$this->model_hcv_product_category->Content($this->request->get['category_id']);}
