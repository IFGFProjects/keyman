<?php
class ControllerCustomerCustomer2 extends Controller {
	private $error = array();
	...
	public function import()
	{
		...
		{
			if(move_uploaded_file($_FILES['import_file']['tmp_name'],DIR_UPLOAD."customer_import/".basename($_FILES['import_file']['name']))) 
			{
				$data = str_getcsv(file_get_contents(DIR_UPLOAD."customer_import/".basename($_FILES['import_file']['name'])), "\n"); //parse the rows
				$header = array_shift($data);
				foreach (str_getcsv($header,';') as $key => $value) 
					{$headers[trim(iconv("cp1251", "utf-8", $value))]=$key;}
				unset($header);
				// echo "<h1 style='color:red;'>headers :</h1><pre>";print_r($headers);echo "</pre><hr>";
				$c=0;
				...
				foreach ($data as &$row) 
				{
					...
					if ($row[$headers['E-mail']]=="")
						{$row[$headers['E-mail']]="nomail_$c@keyman.by";}

					if ($row[$headers['Дата рождения']]!="")
						{$row[$headers['Дата рождения']] = \DateTime::createFromFormat("d.m.Y", $row[$headers['Дата рождения']])->format("Y-m-d");}

					$customer_data=array(
						"customer_group_id"	=> 1,
						"firstname"	=> iconv("cp1251", "utf-8", $row[$headers['Имя']]),
						"lastname"	=> iconv("cp1251", "utf-8", $row[$headers['Фамилия']]),
						"email"	=> $row[$headers['E-mail']],
						"telephone"	=> (int)$row[$headers['Телефон']],
						"fax"	=> "",
						"custom_field"	=> array(1=>$row[$headers['номер карты']],6=>$row[$headers['Дата рождения']]),
						"newsletter"	=> 0,
						"password"	=> "vBwnVhxD",
						"status"	=> 1,
						"approved"	=> 1,
						"safe"	=> 0,
						"add_amount"	=> (float)$row[$headers['Сумма покупок']],
						"address" => array(0=> array(
							"firstname"	=> iconv("cp1251", "utf-8", $row[$headers['Имя']]),
							"lastname"	=> iconv("cp1251", "utf-8", $row[$headers['Фамилия']]),
							"company"	=> "",
							"address_1"	=> iconv("cp1251", "utf-8", $row[$headers['Адрес']]),
							"address_2"	=> "",
							"city"	=> "Минск",
							"postcode"	=> "",
							"country_id"	=> 20,
							"zone_id"	=> 339,
							"custom_field"	=> array(2=>"",3=>"",4=>"",5=>""),
						))
					);
					$this->model_customer_customer_new->addCustomer($customer_data);
					// die();
				    // echo "<h1 style='color:red;'>row :</h1><pre>";print_r($row);echo "</pre><hr>";
				}
			    $data['success']="Успешно добавлено $c покупателей";
			} else 
				{$data['error_warning']="Не удалось загрузить файл";}
		}
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_customer_customer_new->addCustomer($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_customer_customer_new->editCustomer($this->request->get['customer_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $customer_id) {
				$this->model_customer_customer_new->deleteCustomer($customer_id);
			}
	...
	public function approve() {
		...
		if ($customers && $this->validateApprove()) {
			$this->model_customer_customer_new->approve($this->request->get['customer_id']);
	...
	public function unlock() {
		...
		if (isset($this->request->get['email']) && $this->validateUnlock()) {
			$this->model_customer_customer_new->deleteLoginAttempts($this->request->get['email']);
	...
	protected function getList() {
		...
		$data['import'] = $this->url->link('customer/customer/import', 'token=' . $this->session->data['token'], 'SSL');
		...
		$customer_total = $this->model_customer_customer_new->getTotalCustomers($filter_data);

		
		$results = $this->model_customer_customer_new->getCustomers($filter_data);

		$this->load->model("module/bitproapi");
		// $this->model_module_bitproapi->LoadNewCustomers();
		...
		foreach ($results as $result) {
			...
			$login_info = $this->model_customer_customer_new->getTotalLoginAttempts($result['email']);
			...
			$card_number="";
			$birthday="";		
			if (isset($result['custom_field']))
			{
				$add_data=(array)json_decode($result['custom_field'],true);
				if (isset($add_data[1]))
					{$card_number=$add_data[1];}
				if (isset($add_data[6]))
					{$birthday=$add_data[6];}
			}
			...
			$adr=$this->model_customer_customer_new->getAddresses($result['customer_id']);
			$adr=array_pop($adr);

			
			// $client_total=$this->model_module_bitproapi->GetClientTotal($result['customer_id']);
			$client_total=0;
			...
			// $discout_percent=$this->model_module_hbuydiscount->GetDiscountPercent($client_total);
			$discout_percent=0;

			// echo "<h1 style='color:red;'>".$result['customer_id']." :</h1><pre>";print_r();echo "</pre><hr>";
			$data['customers'][] = array(
				...
				'discount'         => 			$discout_percent,
				'address'         => $adr['address_1'],
	...
	protected function getForm() {
		...
		if (isset($this->request->get['customer_id'])) {
			...
			// $this->load->model("module/bitproapi");
			// $client_total=$this->model_module_bitproapi->GetClientTotal($this->request->get['customer_id'],true);
		} else {
			$data['customer_id'] = 0;
		}
		...
		if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$customer_info = $this->model_customer_customer_new->getCustomer($this->request->get['customer_id']);
		}
		...
		if (isset($this->request->post['add_amount'])) {
			$data['add_amount'] = $this->request->post['add_amount'];
		} elseif (!empty($customer_info)) {
			$data['add_amount'] = $customer_info['add_amount'];
		} else {
			$data['add_amount'] = 0;
		}
		...
		if (isset($this->request->post['password'])) {
			...
			$data['password'] = 'vBwnVhxD';
		}

		if (isset($this->request->post['confirm'])) {
			$data['confirm'] = $this->request->post['confirm'];
		} else {
			$data['confirm'] = 'vBwnVhxD';
		}
		...
		if (isset($this->request->post['address'])) {
			...
			$data['addresses'] = $this->model_customer_customer_new->getAddresses($this->request->get['customer_id']);
		} else {
	...
	protected function validateForm() {
		...
		$customer_info = $this->model_customer_customer_new->getCustomerByEmail($this->request->post['email']);
	...
	public function login() {
		...
		$customer_info = $this->model_customer_customer_new->getCustomer($customer_id);

		if ($customer_info) {
			// Create token to login with
			$token = token(64);

			$this->model_customer_customer_new->editToken($customer_id, $token);
	...
	public function history() {
		...
		$results = $this->model_customer_customer_new->getHistories($this->request->get['customer_id'], ($page - 1) * 10, 10);
		...
		$history_total = $this->model_customer_customer_new->getTotalHistories($this->request->get['customer_id']);
	...
	public function addHistory() {
		...
		if (!$this->user->hasPermission('modify', 'customer/customer')) {
			...
			$this->model_customer_customer_new->addHistory($this->request->get['customer_id'], $this->request->post['comment']);
	...
	public function transaction() {
		...
		$results = $this->model_customer_customer_new->getTransactions($this->request->get['customer_id'], ($page - 1) * 10, 10);
		...
		$data['balance'] = $this->currency->format($this->model_customer_customer_new->getTransactionTotal($this->request->get['customer_id']), $this->config->get('config_currency'));

		$transaction_total = $this->model_customer_customer_new->getTotalTransactions($this->request->get['customer_id']);
	...
	public function addTransaction() {
		...
		if (!$this->user->hasPermission('modify', 'customer/customer')) {
			...
			$this->model_customer_customer_new->addTransaction($this->request->get['customer_id'], $this->request->post['description'], $this->request->post['amount']);
	...
	public function reward() {
		...
		$results = $this->model_customer_customer_new->getRewards($this->request->get['customer_id'], ($page - 1) * 10, 10);
		...
		$data['balance'] = $this->model_customer_customer_new->getRewardTotal($this->request->get['customer_id']);

		$reward_total = $this->model_customer_customer_new->getTotalRewards($this->request->get['customer_id']);
	...
	public function addReward() {
		...
		if (!$this->user->hasPermission('modify', 'customer/customer')) {
			...
			$this->model_customer_customer_new->addReward($this->request->get['customer_id'], $this->request->post['description'], $this->request->post['points']);
	...
	public function ip() {
		...
		$results = $this->model_customer_customer_new->getIps($this->request->get['customer_id'], ($page - 1) * 10, 10);

		foreach ($results as $result) {
			$data['ips'][] = array(
				'ip'         => $result['ip'],
				'total'      => $this->model_customer_customer_new->getTotalCustomersByIp($result['ip']),
				'date_added' => date('d/m/y', strtotime($result['date_added'])),
				'filter_ip'  => $this->url->link('customer/customer', 'token=' . $this->session->data['token'] . '&filter_ip=' . $result['ip'], 'SSL')
			);
		}

		$ip_total = $this->model_customer_customer_new->getTotalIps($this->request->get['customer_id']);
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					...
					'address'           => $this->model_customer_customer_new->getAddresses($result['customer_id'])
				);
			}
		}
	...
	public function address() {
		...
		if (!empty($this->request->get['address_id'])) {
			...
			$json = $this->model_customer_customer_new->getAddress($this->request->get['address_id']);
		}
