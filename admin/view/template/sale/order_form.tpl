                  ...
                  <tbody id="cart">
                    ...
                    <tr>
                      <td class="text-left"><?php echo $order_product['name']; ?><br />
                        ...
                        <input type="hidden" name="product[<?php echo $product_row; ?>][option]<?php if ($option['product_kit_id']) {?>[<?php echo $option['product_kit_id']; ?>]<?php } ?>[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option['product_option_value_id']; ?>" />
                        <?php } ?>
                        ...
                        <input type="hidden" name="product[<?php echo $product_row; ?>][option]<?php if ($option['product_kit_id']) {?>[<?php echo $option['product_kit_id']; ?>]<?php } ?>[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option['value']; ?>" />
                        <?php } ?>
                        <?php } ?></td>
...
$('select[name=\'currency\']').on('change', function() {
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				$('#content > .container-fluid').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + ' <button type="button" id="button-ip-add" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-danger btn-xs pull-right"><i class="fa fa-plus"></i> <?php echo $button_ip_add; ?></button> </div>');
...
$('#button-refresh').on('click', function() {
	$.ajax({
		url: $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/cart/products&token=' + token+ '&order_id=' + $('input[name=\'order_id\']').val(),
		dataType: 'json',
		crossDomain: true,
		success: function(json) {
			...
			if (json['products'].length) {
              ...
              var kit_segment='';
              if (option['product_kit_id']!=0)
                {kit_segment="["+option['product_kit_id']+"]";}
							if (option['type'] == 'select' || option['type'] == 'radio' || option['type'] == 'image') {
								html += '<input type="hidden" name="product[' + i + '][option]'+kit_segment+'[' + option['product_option_id'] + ']" value="' + option['product_option_value_id'] + '" />';
							}
							...
							if (option['type'] == 'text' || option['type'] == 'textarea' || option['type'] == 'file' || option['type'] == 'date' || option['type'] == 'datetime' || option['type'] == 'time') {
								html += '<input type="hidden" name="product[' + i + '][option]'+kit_segment+'[' + option['product_option_id'] + ']" value="' + option['value'] + '" />';
							}
						}
					}
