...
class ModelHtagsmgrTags extends Model {
	public function getTags($data=array())
	{
		...
		$tags_main=$this->db->query("
			SELECT tg.tag_id, tg.tag_name as `name`, tg.category_id, cat.name as category_name, tg.link, tg.status, tg.menu_show
	...
	{
		$this->db->query("INSERT INTO " . DB_PREFIX . "htagsmgr_tag SET 
		...
		foreach ($data['tag_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "htagsmgr_tag_description SET 
				tag_id = '" . (int)$tag_id . "', 
	...
	{
		$this->db->query("UPDATE " . DB_PREFIX . "htagsmgr_tag SET 
			tag_name='".$this->db->escape($data['name'])."',
			category_id=".(int)$data['category_id'].",
			link='".$this->db->escape($data['link'])."',
			status=".(int)$data['status'].",
		...
		foreach ($data['tag_description'] as $language_id => $value) {
			$query="UPDATE " . DB_PREFIX . "htagsmgr_tag_description SET 
				description = '" . $this->db->escape($value['description']) . "', 
				meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', 
				meta_title = '" . $this->db->escape($value['meta_title']) . "', 
				meta_description = '" . $this->db->escape($value['meta_description']) . "', 
	...
	{
		$this->db->query("DELETE FROM " . DB_PREFIX . "htagsmgr_tag WHERE tag_id = '" . (int)$tag_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "htagsmgr_tag_description WHERE tag_id = '" . (int)$tag_id . "'");
