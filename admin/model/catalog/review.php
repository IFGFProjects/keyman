...
class ModelCatalogReview extends Model {
	public function addReview($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', email = '" . $this->db->escape($data['email']) . "', product_id = '" . (int)$data['product_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', image = '" . $this->db->escape($data['image']) . "', age = '" . $this->db->escape($data['age']) . "', answer_name = '" . $this->db->escape($data['answer_name']) . "', answer_text = '" . $this->db->escape(strip_tags($data['answer_text'])) . "',  rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
	...
	public function getReviews($data = array()) {
		...
		$sql = "SELECT r.review_id, pd.name, r.author, r.email, r.rating, r.status, r.date_added FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "product_description pd ON (r.product_id = pd.product_id) WHERE true ";
