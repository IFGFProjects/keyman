	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$cfds=$this->db->query("Select * FROM ".DB_PREFIX."custom_field_description WHERE language_id=(SELECT language_id FROM language WHERE `code`='".$this->config->get('config_language')."' LIMIT 1)");
				$replace['addition']='';

				foreach ($cfds->rows as $ckey => $field) 
				{
					if (isset($order_info['payment_custom_field'][$field['custom_field_id']]))
					{$replace['addition'].=$field['name'].": ".$order_info['payment_custom_field'][$field['custom_field_id']]."<br>";}
				}
