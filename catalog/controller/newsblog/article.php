...
class ControllerNewsBlogArticle extends Controller {
	...
	public function index() {
			...
			foreach ($parts as $newsblog_path_id) {
				...
				if ($category_info) {
					$data['breadcrumbs'][] = array(
						...
						'href' => $this->url->link('newsblog/category', 'newsblog_path=' . $newsblog_path, 'SSL')
					);
				}
			}
		...
		if ($article_info) {
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('newsblog/article', $url . '&newsblog_article_id=' . $newsblog_article_id, 'SSL')
			);
			...
			$data['canonical']=$this->url->link('newsblog/article', 'newsblog_path=' . $mainCategoryId . '&newsblog_article_id=' . $newsblog_article_id, 'SSL');
            ...
            $data['href'] = $this->url->link('newsblog/article', 'newsblog_article_id=' . $newsblog_article_id, 'SSL');
			...
			foreach ($results as $result) {
				...
				$data['articles'][] = array(
						...
						'href'        		=> $this->url->link('newsblog/article', 'newsblog_path=' . $mainCategoryId . '&newsblog_article_id=' . $result['article_id'], 'SSL'),
	...
	public function review() {
		...
		$pagination->limit = 5;
		$pagination->url = $this->url->link('newsblog/article/review', 'newsblog_article_id=' . $this->request->get['newsblog_article_id'] . '&page={page}', 'SSL');
