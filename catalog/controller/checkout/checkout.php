...
class ControllerCheckoutCheckout extends Controller { 
	...
	unset($_POST['quantity']);
		...
		//unset($this->session->data['customer_buydiscount']);
		$this->load->model("module/buydiscount");
		if (isset($this->request->post['buydiscount_coupon']))
		{
			$this->session->data['customer_buydiscount']=$this->model_module_buydiscount->GetCouponDiscount($this->request->post['buydiscount_coupon']);			
		} 
		elseif (  (isset($this->session->data['customer_id'])) && (!isset($this->session->data['customer_buydiscount'])) )
		{
			
			$this->session->data['customer_buydiscount']=$this->model_module_buydiscount->GetClientDiscount($this->session->data['customer_id']);
		}

		unset($_POST['buydiscount_coupon']);
	...
	public function confirm($render = true, &$data = array()) {
		...
		if ($opencart2) 
		{
			...
			if (isset($_POST) && !empty($_POST) )
			{
				...
				if ((isset($payment_address) && is_array($payment_address)) || (isset($this->session->data['payment_address']) && isset($this->session->data['payment_address']['firstname']) ) )
				{
						if ($data['opencart2'])
						{
				...
				if ($this->cart->hasShipping()) {
					if(!$this->customer->isLogged())
					{
						if ( (!isset($this->request->post['shipping_address'])) && (isset($this->request->post['firstname'])) )
						{
						...
						elseif (isset($this->request->post['shipping_country_id']))
						{
							$this->load->model('localisation/country');
							$this->load->model('localisation/zone');
							$country_info = $this->model_localisation_country->getCountry($this->request->post['shipping_country_id']);
					...
					}  else
					{
						...
						$order_data['shipping_address_1'] = $order_data['address1'];
						$order_data['shipping_address_2'] = '';
						$order_data['shipping_city'] = 'minsk';
						$order_data['shipping_postcode'] = '220015';
						$order_data['shipping_zone'] = 'Minskaya';
						$order_data['shipping_zone_id'] = '342';
						$order_data['shipping_country'] = 'Belarus';
						$order_data['shipping_country_id'] = '20';
						$order_data['shipping_address_format'] = '';
						$order_data['shipping_custom_field'] = array();
						$order_data['shipping_method'] = 'Доставка с фиксированной стоимостью доставки';
					...
					$order_data['shipping_lastname'] = $order_data['lastname'];
					$order_data['shipping_company'] = '';
					$order_data['shipping_address_1'] = isset($order_data['address1'])?$order_data['address1']:"";
					$order_data['shipping_address_2'] = '';
					$order_data['shipping_city'] = 'minsk';
					$order_data['shipping_postcode'] = '220015';
					$order_data['shipping_zone'] = 'Minskaya';
				...
				if ($order_data['shipping_firstname']!="")
				{
					$this->load->model('checkout/order');
					// echo "<h1 style='color:red;'>order_data :</h1><pre>";print_r($order_data);echo "</pre><hr>";die();
					$this->session->data['order_id'] = $this->model_checkout_order->addOrder($order_data);
					// echo "<h1 style='color:red;'>order_data :</h1><pre>";print_r($this->session->data['order_id']);echo "</pre><hr>";
					// die();
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				//unset($this->session->data['customer_buydiscount']);
				$this->load->model("module/buydiscount");
				// $this->model_module_buydiscount->CountProductPrice($product);
				// if (  (isset($this->session->data['customer_id'])) && (!isset($this->session->data['customer_buydiscount'])) )
				// {
				// 	$this->session->data['customer_buydiscount']=$this->model_module_buydiscount->GetClientDiscount($this->session->data['customer_id']);
				// }
				$price_orig=$this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax'));
				// if (isset($this->session->data['customer_id']))
				// 	if ($this->session->data['customer_buydiscount']>0)
				// {
				// 	$strike_price=$price;
				// 	$price_orig=$this->tax->calculate($product['price']-($product['price']*($this->session->data['customer_buydiscount']/100)), $product['tax_class_id'], $this->config->get('config_tax'));
				// 	$price=$this->currency->format($this->tax->calculate($product['price']-($product['price']*($this->session->data['customer_buydiscount']/100)), $product['tax_class_id'], $this->config->get('config_tax')));
				// }
					$total = $this->currency->format($this->tax->calculate($price_orig, $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']);
