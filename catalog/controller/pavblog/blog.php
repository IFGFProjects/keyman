		...
		public function index() {  
			...
			// крошки  PS:сплошной ГОVНОКОД - ПЕРЕПИСАТЬ !!!
				...
				for ($i = $max_level-1; $i >= 0; --$i) {
						...
						$hleb [] = array(
								'text'      => $result['title'],
								'href'      => $this->url->link('pavblog/category', 'id='.$result['category_id'],'SSL'), 
        	...
        	// крошки END PS:сплошной ГОVНОКОД - ПЕРЕПИСАТЬ !!!	
				...
				$this->mdata['breadcrumbs'][] = array(
					'text'      => $blog['title'],
					'href'      => $this->url->link('pavblog/blog', 'id=' .  $blog_id,'SSL'),      		
					'separator' => $this->language->get('text_separator')
				);	
				...
				if( $blog['image'] ){	
					$this->mdata['image_orig']= $this->model_tool_image->orig($blog['image']);
				...
				$blog['category_link'] =  $this->url->link( 'pavblog/category', "id=".$blog['category_id'] ,'SSL');
				$blog['comment_count'] =  $this->getModel('comment')->countComment( $blog['blog_id'] );
				$blog['link'] =  $this->url->link( 'pavblog/blog','id='.$blog['blog_id'] ,'SSL');
				...
				$this->mdata['comment_action'] = $this->url->link( 'pavblog/blog/comment','id='.$blog['blog_id'] ,'SSL');
				...
				foreach( $ttags as $tag ){
					$tags[trim($tag)] = $this->url->link( 'pavblog/category&id=1&','tag='.trim($tag) ,'SSL');
				}
				...
				$this->mdata['link'] =  $this->url->link( 'pavblog/blog','id='.$blog['blog_id'] ,'SSL');
				...
				$pagination->url = $this->url->link('pavblog/blog', 'id=' . $blog['blog_id'] . $url . '&page={page}','SSL');
				$data = array(
			...
			$lang_tags = $this->model_pavblog_tags->getAllTags();
			...
			foreach ($results as $result) {
				if ($result['image']) {
					...
					$this->mdata['image_orig']= $this->model_tool_image->orig($result['image']);
				} else {
			...
			// товары к статье END
				...
				$children = $this->getModel('category')->getChildren(1);
				...
				//----- END все категории верхнего уровня -----
				...
				if ($category_id==27) {
					if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/pavblog/blog_coll.tpl')) {
						$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/pavblog/blog_coll.tpl', $this->mdata));
					} else {
						$this->response->setOutput($this->load->view('default/template/pavblog/blog_coll.tpl', $this->mdata));
					}
				} else {
